...
unsigned char voiceUsed = 0;
unsigned char vrr;

void polyVoiceAllocate(unsigned char aNote, unsigned char aVelo) {
	unsigned char v;
	
	if (aVelo == 0) { // turn off note
		for (v=0;v<8;v++) {
			if (polyNote[v] == aNote) {
				if (polyGate[v] != 0) {
					polyGate[v] = 0;
