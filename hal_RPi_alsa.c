...
void AudioOut(void) {
	...
	if (samplecount == buffersamples) {
		...
		if (frames < 0) {
			...
			if (snd_pcm_open(&handle, audiodevice, SND_PCM_STREAM_PLAYBACK,0)<0) {
...
//	char audiodevice[100];
	...
	if (paramfil==NULL) {
		printf("System setupfile setting.txt not found!\n");
		return 0;
	}

	fscanf(paramfil,"%s\n%s\n",mididevice, audiodevice);
...
void editor(void) {
#ifdef TEXTEDIT
	printf("Staring the texteditor thread\n");
