...
unsigned char	gate[4];	// 0 first note, 1 sec note, 2 no, arpegg
unsigned char	note[4];	// 0 first note, 1 sec note, 2 no, arpegg

unsigned char	polyNote[8];
unsigned char	polyGate[8];
...
float temperingLUT[128];
float temperingA2Ab[12] = {440, 469.3333333, 495, 528, 550, 583.6666666666667, 618.75, 660, 704, 733.33333333, 792, 825 };
...
void makeNoteToFreqLUT(float masterTune) {
	...
	for (i=69;i<128;i++) {
		noteToFreqLUT[i]=temperingLUT[i]=tempFreq;
		tempFreq*=1.0594631;
	}
	tempFreq=440.0+(masterTune*2.0);
	for (i=68;i>0;i--) {
	...
	for (i=0;i<12;i++) {
		temperingLUT[69-48+i] = temperingA2Ab[i]*0.0625;
		temperingLUT[69-36+i] = temperingA2Ab[i]*0.125;
		temperingLUT[69-24+i] = temperingA2Ab[i]*0.25;
		temperingLUT[69-12+i] = temperingA2Ab[i]*0.5;
		temperingLUT[69+i] 	  = temperingA2Ab[i];
		temperingLUT[69+12+i] = temperingA2Ab[i]*2;
		temperingLUT[69+24+i] = temperingA2Ab[i]*4;
		temperingLUT[69+36+i] = temperingA2Ab[i]*8;
	}
	printf("\n");
	for (i=1;i<128;i++)
		printf("%d\t%f\t%f\t%f\n",i, noteToFreqLUT[i], temperingLUT[i], 1200.0*log2f(noteToFreqLUT[i]/temperingLUT[i]));
	return;
}
